...
% =====================================================================
% The pooling factor predicts cross-validation reliability:
% a closed-form triage and Rao-Blackwellised cure for hierarchical LOO.
% Target: Statistics and Computing (or Bayesian Analysis).
% =====================================================================
%
% !! BEFORE SUBMISSION: REGENERATE ALL EMPIRICAL NUMBERS !!  (2026-08-09)
%
% Deliberately deferred to as late as possible, so that the reported values
% are produced by the same toolchain a referee would get when re-running.
% Two independent reasons, both verified:
%
% 1. r_eff convention (FIXED in the package, numbers here PREDATE the fix).
%    .rb_engine passed chain_id = rep(1L, S) to loo::relative_eff(), i.e. it
%    treated all draws as one chain, while every backend stacks several.
%    r_eff sets the PSIS tail length, so both elpd and k-hat move. Measured
%    on E-real (analysis/measure_reff_convention.R): conditional k-hat moves
%    by up to 0.058, elpd_full by 0.004, and the count of k>0.7 folds changed
%    by one. The saved E-real run has THREE folds within 0.058 of the 0.7
%    cut, so the "five PSIS failures" figure in Section~\ref{sec:ereal} and
%    the AUC 0.84 alongside it can both move. The RB side is essentially
%    immune (elpd_rb moves 1e-5, no fold crosses), so the C2 "cures every
%    failure" claim is robust -- verified 0 RB failures under all three
%    conventions in use anywhere in this repo.
%    NOT affected: Table~\ref{tab:sim}. Its Gaussian row comes from a
%    single-chain Gibbs sampler in analysis/rb_loo.R that additionally thins
%    by 2 ("~independent for loo r_eff=1"), and its logistic row comes from
%    analysis/replicate_glmm.R, which calls loo(fit) and so already used the
%    real chain ids. Those two rows never went through .rb_engine.
%    Affected (they DO call rb_loo()): E-real, E-decision, E-basestress and
%    the multivariate validations.
%
% 2. Toolchain drift, and this is the bigger one. Re-running
%    analysis/ereal_epilepsy.R unchanged, same seed, gives EIGHT PSIS
%    failures where the saved 2026-07-20 run (analysis/ereal_epilepsy.rds)
%    recorded five, with a different max k-hat. The r_eff change accounts
%    for at most one of those. The rest is a different brms/rstan/Stan build
%    producing different draws from the same seed, so E-real as written does
%    not currently reproduce irrespective of item 1.
%
% Action at submission: unify the analysis scripts on the package's engine
% (three r_eff conventions are currently live across analysis/*.R), re-run
% every script that feeds a reported number, and record sessionInfo() or an
% renv lockfile next to the .rds outputs so the next re-run is checkable.
% =====================================================================
...
\title{\bf The pooling factor predicts cross-validation reliability:\\
a closed-form triage and Rao--Blackwellised cure for hierarchical LOO}
...
group whose own data are uninformative about $\alpha_j$ relative to the prior has
$\pi_j$ near $1$ (fully pooled); a group that determines its own effect has
$\pi_j$ near $0$ (unpooled). Note that a singleton in a \emph{diffuse} hierarchy
is the second case, not the first: with $\sigma_u^2$ large, its lone observation
fixes $\alpha_j$ entirely. Having little data and being prior-dominated are
distinct properties, and it is the unpooled singleton that is pathological here.
The per-observation
...
\citet{lovison2026augmented}. The two diagnostics are related by
\[
h_i = I_i\,\sigma_u^2\,\pi_j ,
\]
so they are proportional rather than opposed. The group total $1-\pi_j$ decreases
in $\pi_j$, but it is shared among $n_j = (1-\pi_j)/(I_i\sigma_u^2\pi_j)$
observations, so the per-observation share rises: the sum and the individual share
move in opposite directions. This identity also delimits what $h_i$ contributes.
When the Fisher weight is constant across observations, as in the Gaussian
random-intercept case with $I_i = 1/\sigma^2$, the two rank observations
identically and $h_i$ adds nothing to $\pi_j$. It becomes a distinct diagnostic
only when $I_i$ varies: through the fitted mean in a non-Gaussian family, or
through the design in the random-slope case, where $h_i = z_i'V_jz_i/\sigma^2$ and
$\pi_j = \mathrm{tr}(\Sigma^{-1}V_j)/p$ are different functionals of the same $V_j$.
...
outcomes are seen. By the identity $h_i = I_i\sigma_u^2\pi_j$ of
Section~\ref{sec:prior}, this is the case in which $h_i$ and $\pi_j$ rank
folds identically, so there the triage rests on a single diagnostic wearing two
names. In a GLMM the Fisher weight $I_i$ depends on the fitted mean, so
...
% ---- E-real ----
% REGENERATE BEFORE SUBMISSION (see the header block): the "five PSIS
% failures" and "AUC 0.84" below are the most exposed numbers in the paper.
% Three folds sit within one r_eff-perturbation (0.058) of the 0.7 cut, and
% a plain re-run on the current toolchain already gives eight failures.
...
The experiments here use a single random-intercept grouping factor: Gaussian LMMs
(analytic) and Bernoulli, binomial and Poisson GLMMs (1-D quadrature). Its estimand
is the integrated (marginal-over-RE) predictive, against which we compare throughout
\citep{merkle2019bayesian}. Random slopes and correlated within-group effects extend
the estimator \emph{exactly}, not merely approximately. In the Gaussian case the
block conditional is Gaussian in any dimension, so the leave-one-out downdate becomes
a $p\times p$ matrix downdate (Proposition~\ref{thm:t1}) that stays closed-form and
exact; for GLMMs the multivariate case is a low-dimensional quadrature over the true
conditional (feasible for $p\le3$). The \texttt{rbloo} package implements both, and
we validate them fold-by-fold against brute-force refits (elpd RMSE $\approx0.03$
nats vs the exact refit on a random intercept-plus-slope model, against $\approx0.5$
for PSIS-LOO). Crossed or multiple grouping factors are the genuinely hard case: a
deleted observation loads on several fibers at once and there is no single block to
collapse, so the package falls back to plain PSIS-LOO, which we name as out of scope
...
\item \textbf{The prediction is specifically about the pooling / random-effect
component of reliability.} The pooling factor predicts the LOO failures driven by a
data-driven random effect in a small group; it says nothing about, and explains none
of, the LOO unreliability of a single-level (non-hierarchical) model, where heavy
importance-sampling tails arise from other causes. The claim is confined to
hierarchical models on which the pooling factor is exact (Gaussian) or
well-approximated; we make no claim where it is unavailable (e.g. multiple crossed
grouping factors).
